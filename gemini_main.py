...
3. **결과 매핑 및 감점 (Mapping & Scoring)**:
    1. 사회공학적 조작 및 심리 분석 (40점) - [2025 COLING 논문 근거]
    *가장 정교한 기법으로, 사용자의 이성적인 판단을 마비시키는 항목입니다.
    -비현실적 이득 제안 (Profit-oriented, 15점): "경제적 자유", "마지막 기회", "수백만 원 수익" 등 금전적 이득을 강력하게 암시.
    -동질성 호소 및 직접 화법 (Direct Talk, 15점): "나도 당신 같은 평범한 사람이다", "솔직히 말하겠다"며 친근감을 형성해 경계심을 해제하는 기법.
    -의구심 사전 차단 (Addressing Concerns, 10점): "이건 흔한 돈벌이 약속이 아닙니다", "사기라고 생각하기 전에 일단 들어보세요" 등 사용자의 의심을 미리 예측하고 방어하는 표현.
    
    2. 압박형 다크패턴 분석 (30점) - [다크패턴 가이드라인 근거]
    *소비자의 심리적 압박을 가해 합리적인 선택을 방해하는 항목입니다.
    -허위 시간 제한 알림 (15점): 실제로는 계속 판매함에도 "오늘 마감", "마지막 방송" 등 시간을 제한하여 구매를 재촉하는 행위.
    -허위 재고/수요 알림 (15점): "선착순 10명", "현재 5,000명이 시청 중", "재고 임박" 등 거짓 정보를 통해 경쟁심과 긴박함을 조장하는 행위.

    3. 사회적 증거 조작 및 권위 도용 (20점) - [혼합 근거]
    *신뢰도를 조작하기 위해 검증되지 않은 수치나 인물을 사용하는 항목입니다.
    -사회적 승인 조작 (Social Endorsement, 10점): "85,000명의 한국인이 이미 받았다"는 식의 구체적이지만 검증 불가능한 숫자 제시.
    -가족/지인 사례 인용 (10점): "우리 엄마도 효과를 봤다", "내 지인들도 이미 사용 중이다" 등 개인적인 신뢰 관계를 이용한 감정적 언어 사용.

    4. 언어적 결함 및 정보 은폐 (10점) - [혼합 근거]
    -비문 및 번역투 (5점): 딥페이크나 해외 사기 조직에서 흔히 나타나는 문법적 오류 및 어색한 한국어 표현.
    -숨겨진 정보 및 잘못된 계층 구조 (5점): 중요 정보(부가 조건, 취소 규정 등)를 설명란 아주 하단에 배치하거나, 눈에 띄지 않게 처리하는 행위.
...
4. `patent_check`: **광고에 특허 관련 언급이 있을 경우에만** 작성. 특허 언급이 없으면 반드시 `null`로 설정.
5. `evidence`: 검색을 통해 확인된 핵심 근거만 간략히. 각 `fact`는 1-2문장.
6. `consultation`: **1-2문장**으로 핵심 조언만 제공. 장황한 설명 금지.
7. 'risk_score' : 계산된 최종 의심 성적.
